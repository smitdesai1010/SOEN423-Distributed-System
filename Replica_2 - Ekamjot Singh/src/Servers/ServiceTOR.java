...
public class ServiceTOR implements ServiceInterface{
    private final BackendServerTOR delegateTOR;
    private JSONObject replyObjectJSON;

    private final String SUCCESS = "Success";
    private final String DATA = "Data";
    public ServiceTOR(){
        delegateTOR = new BackendServerTOR();
    ...
    public JSONObject addReservationSlot(String eventID, String eventType, int capacity) {
        delegateTOR.log("REQUEST: addReservationSlot called for eventID " + eventID + " Type: " + eventType + " capacity: " + capacity);
        if(eventID.substring(0,3).equals(delegateTOR._name)){
            if (!delegateTOR._events.containsKey(eventType)) {
                replyObjectJSON.put(SUCCESS, false);
                replyObjectJSON.put( DATA, "The reservation slot is already available");
                delegateTOR.log(replyObjectJSON.get(DATA).toString());
                return replyObjectJSON;
            }

            synchronized (this) {
                if (delegateTOR._events.get(eventType) == null || delegateTOR._events.get(eventType).isEmpty()) {
                    delegateTOR._events.put(eventType, new HashMap<String, String>());
                    delegateTOR._events.get(eventType).put(eventID, Integer.toString(capacity));
            ...
            String[] config = delegateTOR._allServers.get(eventID.substring(0, 3)).split(":");
    ...
    public JSONObject removeReservationSlot(String eventID, String eventType) {
        if(eventID.substring(0,3).equals(delegateTOR._name)){
            delegateTOR.log("REQUEST: removeReservationSlot: " + eventID + " " + eventType);
            if (delegateTOR._events.get(eventType) == null || delegateTOR._events.get(eventType).isEmpty() ||
                    delegateTOR._events.get(eventType).get(eventID) == null ||
                    delegateTOR._events.get(eventType).get(eventID).isEmpty() )
            {
                replyObjectJSON.put(SUCCESS, false);
            ...
            String[] eventDetails = delegateTOR._events.get(eventType).get(eventID).split(":");
            ...
            delegateTOR._events.get(eventType).remove(eventID);
            replyObjectJSON.put(SUCCESS, true);
    ...
    public JSONObject listReservationSlotAvailable(String eventType) {
        delegateTOR.log("REQUEST: listReservationSlotAvailable for event Type " + eventType + "");
        broadcastRequestUDP("getReservationsSlots:" + eventType);
        ArrayList<String> s = ClientUDP._responses;
        s.add(delegateTOR._events.get(eventType).toString());
    ...
    public JSONObject reserveTicket(String participantID, String eventID, String eventType) {
        delegateTOR.log("REQUEST: reserveTicket for " + participantID + " " + eventID + " " + eventType);
        ...
        if (checkBookingLimit(participantID, eventID, eventType)){
            if(eventID.substring(0,3).equals(delegateTOR._name)){
                success =  delegateTOR.bookSlot(participantID, eventType, eventID);
            }
            else{
                String[] config = BackendServerTOR._allServers.get(eventID.substring(0,3)).split(":");
                ...
                //    delegateTOR.log(Boolean.toString(s == null));
                delegateTOR.log(s.get(0));
    ...
    public JSONObject getEventSchedule(String ParticipantID) {
        delegateTOR.log("REQUEST: getEventSchedule " + ParticipantID);
        String[] schedule = null;
//        if (ParticipantID.substring(0, 3).equals(delegateTOR._name)) {
//            schedule = new String[1];
//            HashMap<String, String> booking = delegateTOR._participantBookings.get(ParticipantID);
...
//        }else{
        ...
        HashMap<String, String> booking = delegateTOR._participantBookings.get(ParticipantID);
        schedule[0] = delegateTOR._name + " ==> " + (booking != null ? booking.toString() : "{}");
        ArrayList<String> s = ClientUDP._responses;
        Object[] objs = s.toArray();
    ...
    public JSONObject cancelTicket(String participantID, String eventID) {
        delegateTOR.log("REQUEST: cancelTicket " + participantID + " " + eventID);
        String calEventType = "";
        if(eventID.substring(0, 3).equals(delegateTOR._name)) {
            delegateTOR.log("there 0");
            delegateTOR.log(delegateTOR._participantBookings.toString());
            if(delegateTOR._participantBookings.containsKey(participantID)) {
                //delegateTOR.log("there 00");
                for(String eventType : BackendServer.eventTypes) {
                    String temp = delegateTOR._participantBookings.get(participantID).get(eventType);
                    if(temp ==null || temp.length() < 1) continue;
                    int idx = temp.indexOf(eventID);
                    if(idx != -1) {
                        delegateTOR._participantBookings.get(participantID).put(eventType,removeID(temp,eventID));
                        calEventType = eventType;
                        delegateTOR.log("there 1");
                        break;
                    }
                }
        ...
        // if(eventID.substring(0, 3).equals(delegateTOR._name)) {
        //delegateTOR.log("there 2");
        if (eventID.substring(0, 3).equals(delegateTOR._name)) {
            delegateTOR.log("there 3");
            String temp = delegateTOR._events.get(calEventType).get(eventID);
            //temp = temp.substring(temp.indexOf(":") != -1 ? temp.indexOf(":") : 0);
            int idx = temp.indexOf(participantID);
            if(idx != -1) {
                delegateTOR._events.get(calEventType).put(eventID, removeID(temp,participantID));
                // delegateTOR._events.get(calEventType).put(eventID, "");
                replyObjectJSON.put(SUCCESS,true);
            ...
            // delegateTOR.log("there 4 " + eventID.substring(0, 3));
            String[] config = BackendServerTOR._allServers.get(eventID.substring(0,3)).split(":");
    ...
    public boolean verifyUser(String userID, boolean isAdmin) {
        delegateTOR.log("REQUEST: verification request for userID " + userID);
        return isAdmin ? delegateTOR._admins.contains(userID) : delegateTOR._participants.contains(userID);
    }


    @Override
    public JSONObject exchangeTicket(String participantID, String eventID, String newEventID, String newEventType) {
        delegateTOR.log("REQUEST: exchangeTickets for " + participantID + " " + eventID + " " + newEventID
                +  " " + newEventType);
        if(!participantID.substring(0,3).equals(delegateTOR._name)){
            replyObjectJSON.put(SUCCESS,false);
    ...
    public int getCapacity(String eventID, String eventType) {
        if(eventID.substring(0,3).equals(delegateTOR._name)){
            return delegateTOR.getCapacity(eventID,eventType);
        }else{
            String[] config = BackendServerTOR._allServers.get(eventID.substring(0,3)).split(":");
    ...
    private boolean hasTicket(String participantID, String eventID) {
        if(eventID.substring(0,3).equals(delegateTOR._name))
            return delegateTOR.getAllEventIds(participantID).indexOf(eventID) != -1;
    ...
    private void broadcastRequestUDP(String request) {
        Set<String> servers = BackendServerTOR._allServers.keySet();
        ClientUDP[] allThreads = new ClientUDP[servers.size() - 1];

        int ctr = 0;
        for (String server : servers) {
            if (server.equals(delegateTOR._name))
                continue;
            String[] config = BackendServerTOR._allServers.get(server).split(":");
    ...
    private void sendRequestUDP(String request, String Destination) {
        Set<String> servers = BackendServerTOR._allServers.keySet();
        ClientUDP[] allThreads = new ClientUDP[servers.size() - 1];
        String[] config = BackendServerTOR._allServers.get(Destination).split(":");
    ...
    private boolean checkBookingLimit(String participantID, String eventID, String eventType) {
        return true;
//        if (delegateTOR._participantBookings.get(participantID) == null)
//            return true;
...
//
//        String temp = delegateTOR._participantBookings.get(participantID).get("ArtGallary");
//        String bookings = temp == null ? "" : temp;
//        temp = delegateTOR._participantBookings.get(participantID).get("Concerts");
//        bookings += temp == null ? "" : temp;
//        temp = delegateTOR._participantBookings.get(participantID).get("Theatre");
//        bookings += temp == null ? "" : temp;
...
//            if (allBookings[i].substring(0, allBookings[i].length() - 7) == delegateTOR._name) {
//                continue;
//            }
//
//            year = Integer
